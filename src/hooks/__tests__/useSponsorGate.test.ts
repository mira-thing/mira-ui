...
describe('useSponsorGate', () => {
  ...
  it('keeps counting when overlays open and close mid-timer', () => {
    // every overlay open hands the hook fresh shown/onShow identities
    const onShow = vi.fn()
    const { rerender } = renderHook((p: UseSponsorGateParams) => useSponsorGate(p), {
      initialProps: params({ onShow }),
    })

    const slice = SPONSOR_AFTER_PLAY_MS / 6
    for (let i = 0; i < 6; i++) {
      advance(slice)
